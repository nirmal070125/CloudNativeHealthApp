...
service / on new http:Listener(9090) {
    ...
    isolated resource function get fhir/r4/metadata() returns json|http:InternalServerError {
        handlers:IssueHandler issueHandler = new("Service");
        handlers:LogHandler logHandler = new("Service");

        json|error reponse = capabilityStatement.toJson();

        if(reponse is json) {
            logHandler.Debug("Capability statement served at " + time:utcNow()[0].toString());
            return reponse;                
        } else {
            issueHandler.addServiceError(createServiceError(constants:FATAL, constants:PROCESSING, reponse, constants:INTERNAL_SERVER_ERROR));
            return handleServiceErrors(issueHandler);
        }
    }
}
